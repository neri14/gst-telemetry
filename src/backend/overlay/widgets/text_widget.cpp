...
std::shared_ptr<TextWidget> TextWidget::create(parameter_map_ptr parameters) {
    ...
    for (const auto& [name, param] : *parameters) {
        if (name == "x") {
            ...
            log.warning("Unknown parameter '{}' for TextWidget", name);
        }
    }

    if (!widget->x_ || !widget->y_ || !widget->value_ ) {
...
void TextWidget::draw(time::microseconds_t timestamp, cairo_t* cr,
    ...
    if (visible_->get_value(timestamp)) {
        ...
        if (cache_update_needed) {
            std::string text = value_->get_value(timestamp);
...
void TextWidget::draw_text(cairo_t* cr, int width, int height, int margin,
