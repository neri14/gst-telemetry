...
DatetimeWidget::DatetimeWidget(std::shared_ptr<track::Track> track,
                               int x,
                               int y,
                               ETextAlign align,
...
void DatetimeWidget::draw(time::microseconds_t timestamp, cairo_t* cr) {
    std::string str = "-";
    track::Value value = track_->get(key_id_, timestamp);

    if (value.is_valid() && value.is_time_point()) {
        auto tp = value.as_time_point();
        auto tpcast = std::chrono::time_point_cast<std::chrono::seconds>(tp);
        auto tpzoned = std::chrono::zoned_time{timezone_, tpcast};
        str = std::vformat(format_, std::make_format_args(tpzoned));
    }

    //TODO draw to cache and use cached surface - invalidate cache only if str changes
    //TODO create common text drawing function for TextWidget and DatetimeWidget (and others?)
